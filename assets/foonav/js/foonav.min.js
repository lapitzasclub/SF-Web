	...
	window.FooNav = {
		defaults: {
			...
			icons: {
				...
				menu: { family: 'fa', icon: 'fa-sitemap' },
			...
			menu: function(){
				...
				function _build(menu, parent, items){
					...
					if (parent != null){
						...
						$item = $('<a></a>', { 'class': 'fon-item fon-item-back', href: parent.href, text: parent.text, onclick: parent.onclick })
							.prepend(_icon('back'))
							.appendTo($li);
					...
					for (i = 0; i < l; i++){
						item = items[i];
						$li = $('<li></li>').appendTo(menu);
						$item = $('<a></a>', { 'class': 'fon-item fon-item-link', href: item.href, text: item.text, onclick: item.onclick })
							.appendTo($li);
