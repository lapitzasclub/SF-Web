...
function getSteamGroupEvents(from, to) {
  calendarEvents = [];
  var eventsPending = 0;
    ...
    for (var month = from.getMonth(); month <= endMonth; month++) {
      ...
      $.getJSON(yqlURL, function(data) {
        ...
        arrEvents.each(function(i, e) {
          ...
          var eval = year + '-' + month + '-' + day + ' ' + hours + ':' + minutes;
          var time = moment.tz(eval, 'YYYY-MM-DD HH:mm', 'America/Los_Angeles');
...
function fncNextEvent(event, date, end) {
  if (date == null) {
    date = new Date;
  }
  if (end == null) {
